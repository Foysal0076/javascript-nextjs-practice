...
const DragAndReorder = () => {
    ...
    function onDragEnd (result) {
        ...
        if (sourceItemContainerId === destinationItemContainerId) {
            // For same column
            const items = reorder(state[sourceItemContainerId], source.index, destination.index)
            const newState = [...state]
            newState[sourceItemContainerId] = items
            setState(newState)
        } else {
            // For different column
            const result = move(state[sourceItemContainerId], state[destinationItemContainerId], source, destination)
            const newState = [...state]
    ...
    return (
        <Box my={4}>
            <Typography component='h1' variant="h2" textAlign='center' mb={3}>Drag and Reorder</Typography>
            <div>
                <button
                    type="button"
                    onClick={() => {
